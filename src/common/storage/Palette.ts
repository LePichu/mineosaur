...
export class DataPalette {
    ...
    private storage: BitStorage|null;
	...
	constructor(palette: Palette, storage: BitStorage | null, paletteType: PaletteType, globalPaletteBits: number) {
	...
	static writeSize(palette: DataPalette) {
		if (palette.palette instanceof SingletonPalette) {
            return 1 + VarInts.getVarIntSize(0) + VarInts.getVarIntSize(palette.palette.idToState(0));
        }
        ...
        if (!(palette.palette instanceof GlobalPalette)) {
            const paletteLength = palette.palette.size();
            size += VarInts.getVarIntSize(paletteLength);
            for (let i = 0; i < paletteLength; i++) {
                size += VarInts.getVarIntSize(palette.palette.idToState(i));
            }
        }

		size += VarInts.getVarIntSize(palette.storage.getPacketSize());
